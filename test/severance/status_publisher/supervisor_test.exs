defmodule Severance.StatusPublisher.SupervisorTest do
  ...
  describe "reload/0" do
    ...
    test "returns the failed publisher name when one fails to start" do
      parent = self()

      Application.put_env(:severance, :publishers, %{
        good: %{fn: fn _ -> send(parent, :good_tick) end, interval_ms: 20}
      })

      start_supervised!({Severance.Countdown, shutdown_time: ~T[23:59:59]})
      start_supervised!(Severance.StatusPublisher.Supervisor)

      assert_receive :good_tick, 200

      Application.put_env(:severance, :publishers, %{
        good: %{fn: fn _ -> send(parent, :good_tick) end, interval_ms: 20},
        bad_name: %{interval_ms: 20}
      })

      assert {:ok, 1, [:bad_name]} = Severance.StatusPublisher.Supervisor.reload()
